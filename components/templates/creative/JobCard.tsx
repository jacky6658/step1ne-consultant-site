...
export default function CreativeJobCard({ job, onApply, onViewDetail, accentColor = "#f59e0b" }: JobCardProps) {
  const skills = job.key_skills ? job.key_skills.split(/[,;、]/).map((s) => s.trim()).filter(Boolean).slice(0, 5) : [];

  return (
    <div className="group relative rounded-2xl p-6 bg-white border-2 border-gray-100 hover:border-transparent hover:shadow-xl transition-all duration-300 overflow-hidden cursor-pointer" onClick={() => onViewDetail?.(job)}>
